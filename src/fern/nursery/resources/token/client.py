...
class TokenClient:
    ...
    def get_tokens_for_owner(self, owner_id: OwnerId) -> typing.List[TokenMetadata]:
        _response = httpx.request("GET", urllib.parse.urljoin(f"{self._environment}/", f"tokens/owner/{owner_id}"))
        try:
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
    ...
    def revoke_token_by_id(self, token_id: TokenId) -> None:
        _response = httpx.request("POST", urllib.parse.urljoin(f"{self._environment}/", f"tokens/revoke/{token_id}"))
        if 200 <= _response.status_code < 300:
            return
        try:
            _response_json = _response.json()
